Building Elements Core with Visual Studio
=========================================

Introduction
---------------------
Solution and project files to build Elements Core with `msbuild` or Visual Studio can be found in the `build_msvc` directory. The build has been tested with Visual Studio 2019 (building with earlier versions of Visual Studio should not be expected to work).

To build Elements Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build Elements Core with the GUI, a static build of Qt is required.
...
To build Elements Core without Qt, unload or disable the `elements-qt`, `libbitcoin_qt` and `test_bitcoin-qt` projects. The latter two names are internal library/test project names.
...
```cmd
msbuild -property:Configuration=Release -maxCpuCount -verbosity:minimal elements.sln
```

Alternatively, open the `build_msvc/elements.sln` file in Visual Studio 2019.

Security
---------------------
[Base address randomization](https://docs.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization?view=msvc-160) is used to make Elements Core more secure. When building Elements using the `build_msvc` process, base address randomization can be disabled by editing `common.init.vcxproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `elementsd` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers x64\Release\elementsd.exe
```
