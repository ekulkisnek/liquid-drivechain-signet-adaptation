...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        if (model->wallet().hasExternalSigner()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed Elements Transaction (PSET) for use with an offline %1 wallet or PSET-compatible signing device.").arg(CLIENT_NAME));
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    if (model->wallet().privateKeysDisabled() && !model->wallet().hasExternalSigner()) {
        ...
        question_string.append(tr("Please review your transaction proposal. This will produce a Partially Signed Elements Transaction (PSET), which you can save or copy and then sign with an offline %1 wallet or PSET-compatible signing device.").arg(CLIENT_NAME));
        ...
        question_string.append(tr("Please review your transaction. You can send it now or create a Partially Signed Elements Transaction (PSET), which you can save or copy and then sign with an offline %1 wallet or PSET-compatible signing device.").arg(CLIENT_NAME));
    } else {
