...
namespace common {
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn, bool require_production_chain)
{
    try {
        ...
        // Reject alternate identities before creating settings or wallet directories.
        if (require_production_chain) EnsureElementsProductionChain(args);
