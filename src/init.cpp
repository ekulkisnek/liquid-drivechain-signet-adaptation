...
{
    return CallMainChainRPCChecked("getblockcount", UniValue(UniValue::VARR)).get_int64();
}
                             ...
                             std::string* error)
{
    ...
    uint256 parsed;
    if (!ParseHashStr(*configured, parsed)) {
...
static void SubmitDrivechainBmmGrpcRequest(const int sidechain_slot, const int64_t mainchain_tip_height, const uint256& mainchain_tip_hash, const uint256& critical_hash, const CAmount sidechain_fees)
{
#ifdef WIN32
...
static bool SubmitNativeDrivechainBmm(const int sidechain_slot, const int64_t parent_height, const uint256& parent_hash, const uint256& critical_hash, const CAmount sidechain_fees)
{
    ...
    try {
        SubmitDrivechainBmmGrpcRequest(sidechain_slot, mainchain_tip_height, mainchain_tip_hash, critical_hash, sidechain_fees);
        return true;
...
static bool WaitForDrivechainBmmCommitment(const uint256& critical_hash, const uint256& parent_hash, const int sidechain_slot)
{
    int attempts = 0;
...
static bool MineNativeBlockForParentBlock(NodeContext& node, const int64_t parent_height, const uint256& parent_hash)
{
    ...
    if (!SubmitNativeDrivechainBmm(sidechain_slot, parent_height, parent_hash, critical_hash, sidechain_fees)) {
    ...
    if (!WaitForDrivechainBmmCommitment(critical_hash, parent_hash, sidechain_slot)) {
        return false;
    }
...
bool AppInitBasicSetup(const ArgsManager& args)
{
    ...
    if (!CheckCanonicalUsddSp1VerifierStartupIdentity(&identity_error)) {
        return InitError(strprintf(Untranslated(
            "Refusing to start with an incompatible USDD SP1 verifier: %s"),
            identity_error));
    }
#endif
#ifdef _MSC_VER
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        if (native_drivechain) {
            // Both crash-recovery replay in LoadChainstate and level-4
            // verification reconnect blocks under cs_main. Those paths must
            // consume authenticated parent state, never initialize its
            // potentially large replay store while holding consensus locks.
            // Do this before opening/mutating the child databases, including
            // on a reindex retry. Parent outages are not child DB corruption
            // and must not trigger the database-rebuild question below.
            ...
            if (!WarmDrivechainParentState(&warm_error) ||
                ...
                return InitError(Untranslated(strprintf(
                    "ERROR: cannot authenticate drivechain parent state before loading the child chain: %s",
                    warm_error.empty() ? "no authenticated replay generation" : warm_error)));
            }
        }
        ...
        if (maybe_load_error.has_value()) {
            ...
            if (native_drivechain) {
                // The parent can reorganize while this process is stopped or
                // while the child block index is loading. Reauthenticate and
                // reconcile persisted anchors before VerifyDB tries to
                // reconnect their blocks. Retain the final pre-P2P check as
                // well: neither a stale cache nor an orphaned anchor is a
                // reason to bypass BIP301 checks or discard the child DB.
                std::string warm_error;
                if (!WarmDrivechainParentState(&warm_error)) {
                    return InitError(Untranslated(strprintf(
                        "ERROR: cannot authenticate drivechain parent state before verifying the child chain: %s",
                        warm_error)));
                }
                BlockValidationState anchor_state;
                if (!chainman.ActiveChainstate().ReconcileDrivechainAnchorsForStartup(anchor_state)) {
                    return InitError(Untranslated(strprintf(
                        "ERROR: cannot reconcile drivechain anchors before verifying the child chain: %s",
