...
BOOST_AUTO_TEST_CASE(alpha_explicit_only_height_is_frozen)
{
    const auto alpha = CChainParams::ElementsDrivechain();
    BOOST_CHECK_EQUAL(alpha->GetConsensus().explicit_only_height, 84);
    BOOST_CHECK(!alpha->GetConsensus().ExplicitOnlyActive(83));
    BOOST_CHECK(alpha->GetConsensus().ExplicitOnlyActive(84));
    BOOST_CHECK(alpha->GetConsensus().ExplicitOnlyActive(85));
}
