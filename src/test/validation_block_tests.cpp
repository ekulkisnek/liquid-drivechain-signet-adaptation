...
BOOST_FIXTURE_TEST_CASE(explicit_gate_disk_reindex, ElementsModeGateSetup)
{
    ...
    // Retain it with the gate inactive or disabled (Alpha), then reject it
    // when the isolated fixture explicitly opts into the restrictive rule.
    for (int activation : {2, -1, 1}) {
        m_node.validation_signals->SyncWithValidationInterfaceQueue();
        ...
        BOOST_CHECK_EQUAL(WITH_LOCK(cs_main, return m_node.chainman->ActiveHeight()), activation != 1 ? 1 : 0);
        if (activation != 1) {
